...
class Workout {
  ...
  List<ExecutableStep> expand() {
    ...
    bool containsNestedRepeat(RepeatGroup group) {
      return group.steps.any((node) => node is RepeatGroup);
    }
    ...
    void walk(
      ...
      for (final node in nodes) {
        if (node is WorkoutStep) {
          ...
          // Sau step đầu tiên, các step tiếp theo trong cùng round
          // không còn được coi là "first step of round".
        ...
        if (node is RepeatGroup) {
          ...
          for (var round = 1; round <= node.repeat; round++) {
            // Nếu repeat này chứa nested repeat thì không announce
            // vòng ngoài. Inner repeat sẽ tự tạo context riêng.
