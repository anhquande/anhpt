...
class LocalStore {
  ...
  static const _workoutSessions = 'anhpt.workoutSessions.v1';
  ...
  Future<List<WorkoutSession>> loadWorkoutSessions() async {
    final p = await SharedPreferences.getInstance();
    final raw = p.getString(_workoutSessions);
    if (raw == null) return [];
    try {
      ...
      for (final value in decoded) {
        try {
          ...
          // Keep one malformed record from hiding the rest of the history.
        }
      }
  ...
  Future<void> markWorkoutSeen(String workoutId) async {
    final p = await SharedPreferences.getInstance();
    final ids = (p.getStringList(_seenWorkoutIds) ?? const []).toSet()
      ..add(workoutId);
  ...
  Future<bool> isOnboarded() async {
    ...
    final hasExistingInstallData = p.getKeys().any(existingInstallKeys.contains);
  ...
  Future<Map<String, CoachRecording>> loadCoachRecordings() async {
    final p = await SharedPreferences.getInstance();
    final raw = p.getString(_coachRecordings);
    if (raw == null) return {};
    final decoded = Map<String, dynamic>.from(jsonDecode(raw) as Map);
    return decoded.map(
      (key, value) => MapEntry(
        key,
        CoachRecording.fromJson(Map<String, dynamic>.from(value as Map)),
      ),
    );
  }

  Future<void> saveCoachRecordings(
    Map<String, CoachRecording> recordings,
  ) async {
    final p = await SharedPreferences.getInstance();
    await p.setString(
      _coachRecordings,
      jsonEncode(recordings.map((k, v) => MapEntry(k, v.toJson()))),
    );
  }
  ...
  Future<void> saveMusicTracks(List<MusicTrack> tracks) async {
    ...
    await p.setString(
      _musicTracks,
      jsonEncode(tracks.where((e) => !e.bundled).map((e) => e.toJson()).toList()),
    );
  }
  ...
  Future<Map<String, WorkoutMusicConfig>> loadWorkoutMusic() async {
    ...
    return d.map(
      (k, v) => MapEntry(
        k,
        WorkoutMusicConfig.fromJson(Map<String, dynamic>.from(v as Map)),
      ),
    );
  }

  Future<void> saveWorkoutMusic(
    Map<String, WorkoutMusicConfig> configs,
  ) async {
    final p = await SharedPreferences.getInstance();
    await p.setString(
      _workoutMusic,
  ...
  Future<List<WorkoutBucketSource>> loadBucketSources() async {
    final p = await SharedPreferences.getInstance();
    final raw = p.getString(_bucketSources);
    final sources = raw == null
        ? <WorkoutBucketSource>[]
        : (jsonDecode(raw) as List)
            .map(
              (v) => WorkoutBucketSource.fromJson(
                Map<String, dynamic>.from(v as Map),
              ),
            )
            .toList();
    final seeded = p.getBool(_defaultBucketSourceSeeded) ?? false;
    if (seeded) return sources;
    final result = sources.any((s) => s.catalogUrl == defaultBucketSourceUrl)
        ? sources
        : [
    ...
    await p.setBool(_defaultBucketSourceSeeded, true);
    return result;
  }
  ...
  Future<List<InstalledWorkoutProvenance>> loadInstalledBucketWorkouts() async {
    final p = await SharedPreferences.getInstance();
    final raw = p.getString(_installedBucketWorkouts);
    if (raw == null) return [];
    return (jsonDecode(raw) as List)
        .map(
          (v) => InstalledWorkoutProvenance.fromJson(
            Map<String, dynamic>.from(v as Map),
          ),
        )
        .toList();
  ...
  Future<void> saveInstalledBucketWorkouts(
    List<InstalledWorkoutProvenance> workouts,
  ) async {
    final p = await SharedPreferences.getInstance();
    await p.setString(
      _installedBucketWorkouts,
